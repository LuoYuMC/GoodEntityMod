package net.luoyu.goodentity.client.renderer.layers;
...
public class LuoYuWolfEmissiveBodyLayer<M extends AdvancedHumanoidModel<T>, T extends LuoYuWolfEntity> extends EyesLayer<T, M> implements FirstPersonLayer<T> {
    private final RenderType renderType;
    private final RenderType renderType2;

    public LuoYuWolfEmissiveBodyLayer(RenderLayerParent<T, M> layerParent, ResourceLocation emissiveTexture) {
        super(layerParent);
    ...
    public void renderFirstPersonOnArms(PoseStack stack, MultiBufferSource bufferSource, int packedLight, T entity, HumanoidArm arm, PartPose armPose, float partialTick) {
        ...
        if (entity.shouldShowGlow()) {
            ...
            FormRenderHandler.renderModelPartWithTexture(model.m_102851_(arm), stack, bufferSource.getBuffer(this.renderType()), LightTexture.FULL_BRIGHT, 1.0F);
            stack.popPose();
        }
    }
}
